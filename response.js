...
import CacheRequestManager from "./cacheRequestManager.js";
...
export default class Response {
    ...
    JSON(obj, ETag = "",fromchache = false) { 
                         // ok status with content
        if (ETag != ""){    
            this.res.writeHead(200, { 'content-type': 'application/json', 'ETag': ETag });
            if(!fromchache && this.HttpContext.path.isAPI && this.HttpContext.path.id == undefined){
                CacheRequestManager.add(this.HttpContext.req.url,obj,ETag);
            }
        }
        else
